export default async function ({ addon, console, settings }) {
   ...
   */
    ...
    const globalHandleDragOver = (e) => {
      ...
      (el = e.target.closest('div[class*="selector_wrapper"]'))
            ...
            callback = (files) => {
                const hdFilter = settings.get('use-hd-upload') ? '' : ':not(.sa-better-img-uploads-input)';
                const fileInput = el.querySelector('input[class*="action-menu_file-input"]' + hdFilter);
                fileInput.files = files;
                fileInput.dispatchEvent(new Event('change', { bubbles: true }));
            };
        } else if (
            !addon.redux.state.scratchGui.mode.isPlayerOnly &&
      (el = e.target.closest('div[class*="monitor_list-monitor"]'))
        ) {
            callback = (files) => {
                const contextMenuBefore = document.querySelector('body > .react-contextmenu.react-contextmenu--visible') as HTMLElement | null;
                // Simulate a right click on the list monitor
                el.dispatchEvent(new MouseEvent('contextmenu', { bubbles: true }));
                // Get the right click menu that opened (monitor context menus are
                // Children of <body>)
                const contextMenuAfter = document.querySelector('body > .react-contextmenu.react-contextmenu--visible') as HTMLElement | null;
                ...
                (contextMenu.children[0] as HTMLElement).click();
            };
        } else if (
            (el = e.target.closest('div[class*="question_question-input"] > input[class*="input_input-form_l9eYg"]'))
        ) {
            callback = async (files) => {
                const text = (await Promise.all(Array.from(files, (file: File) => file.text())))
                    .join('')
    ...
    document.addEventListener('dragover', globalHandleDragOver, { capture: true });
    return () => {
        document.removeEventListener('dragover', globalHandleDragOver, { capture: true });
    };
}
