import WorkspaceQuerier, { QueryResult } from './WorkspaceQuerier';
import renderBlock, { BlockComponent, getBlockHeight } from './BlockRenderer';
import { BlockInstance, BlockShape, BlockTypeInfo } from './BlockTypeInfo';
import { onClearTextWidthCache } from './module';

export default async function ({ addon, intl, console, settings }) {
    ...
    popupRoot.dir = 'ltr'/*Addon.api.direction*/;
    ...
    popupInputContainer.classList.add(addon.api.hashedScratchClass('input_input-form'));
   ...
   */
    ...
    function openPopup () {
        ...
        // If (addon.api.editorMode !== 'editor') return;
        if (addon.redux.state.scratchGui.editorTab.activeTabIndex !== 0) return;

        blockTypes = BlockTypeInfo.getBlocks(Blockly, vm, Blockly.getMainWorkspace(), intl);
