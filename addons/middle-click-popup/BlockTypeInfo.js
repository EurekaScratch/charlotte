...
export class BlockTypeInfo {
   ...
   * @param {*} vm
   * @param {*} workspace
   * @param {IntlShape} intl The intl instance
   * @returns {BlockTypeInfo[]}
   */
    static getBlocks (Blockly, vm, workspace, intl) {
    ...
    static _createBlocks (workspace, vm, Blockly, intl, workspaceForm, domForm) {
        const parts = [];
        const inputs = [];
        ...
        const addFieldInputs = (field, inputIdx, fieldIdx) => {
            if (field.className_ === 'blocklyText blocklyDropdownText') {
                ...
                switch (field.src_.split('/').pop()) {
                    // Remove msg temporaily
                    case 'green-flag.svg':
                        parts.push('/_general/blocks/green-flag');
                        break;
                    case 'rotate-right.svg':
                        parts.push('/_general/blocks/clockwise');
                        break;
                    case 'rotate-left.svg':
                        parts.push('/_general/blocks/anticlockwise');
                        break;
                }
